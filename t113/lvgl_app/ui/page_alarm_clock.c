...
static void deinit_refresh_timer(void)
{
...
static void back_btn_click_event_cb(lv_event_t * e){
    is_cancle = true;
    deinit_refresh_timer();
    delete_current_page(&com_style);
    init_page_main();
}

static void tomato_alarm_click_event_cb(lv_event_t * e){
    is_cancle = true;
    deinit_refresh_timer();
    delete_current_page(&com_style);
...
static lv_obj_t * init_tomato_alarm_label(lv_obj_t *parent){
    lv_obj_t *label = lv_label_create(parent);
    obj_font_set(label,FONT_TYPE_CN, FONT_SIZE_TEXT_1);
    lv_label_set_text(label,"番茄闹钟");
    lv_obj_set_style_text_color(label,APP_COLOR_WHITE,0);
    lv_obj_add_flag(label,LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_event_cb(label,tomato_alarm_click_event_cb,LV_EVENT_CLICKED,NULL);
    lv_obj_align(label,LV_ALIGN_BOTTOM_LEFT,40,-30);
    return label;
}
