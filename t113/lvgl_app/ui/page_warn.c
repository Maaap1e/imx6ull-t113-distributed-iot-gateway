...
static warning_info_t warning_info_list[] = {
    ...
    {WARN_FLAME_SENSOR_TRIGGER,"主人,你是不是忘记关火了？",GET_IMAGE_PATH("icon_flame_sensor.png")},
        {WARN_SENSOR_DATA_TRIGGER,"传感器异常",GET_IMAGE_PATH("icon_alarm.png")},
};

static lv_style_t com_style;
static lv_obj_t * page = NULL;
static char sensor_warning_message[128] = "传感器异常";

void set_sensor_warning_message(const char *msg)
{
    if(msg == NULL || strlen(msg) == 0){
        strcpy(sensor_warning_message,"传感器异常");
        return;
    }
    strncpy(sensor_warning_message,msg,sizeof(sensor_warning_message) - 1);
    sensor_warning_message[sizeof(sensor_warning_message) - 1] = '\0';
}
...
static lv_obj_t * init_message_view(lv_obj_t *parent,WRANING_TYPE_E type){
    ...
    if(type == WARN_SENSOR_DATA_TRIGGER){
        lv_label_set_text(msg,sensor_warning_message);
