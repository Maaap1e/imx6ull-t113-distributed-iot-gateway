...
static node_status_view_t node_status_views[NODE_STATUS_COUNT];
...
typedef struct {
    ...
    int volume_muted;
    int alarm_active;
} main_refresh_cache_t;

static main_refresh_cache_t refresh_cache = {-1,-1,-1,-1};

#define NODE_COLOR_ONLINE  0x59D878
#define NODE_COLOR_OFFLINE 0xF06464
#define NODE_COLOR_UNKNOWN 0x7F8792
...
static bool menu_item_visible(MENU_TYPE_E type)
{
    return type != MENU_LINKAGE &&
           type != MENU_TOMATO_TIME &&
           type != MENU_CITY &&
           type != MENU_LANGUAGE &&
           type != MENU_APPLE &&
           type != MENU_ABOUT;
...
static void set_node_status(node_status_id_t id,
                            const char *state_text,
                            uint32_t color)
{
    node_status_view_t *view;

    if(id >= NODE_STATUS_COUNT)
        return;

    view = &node_status_views[id];
    if(view->dot == NULL || view->state_label == NULL)
        return;
    if(view->cached_color == color && strcmp(view->cached_text,state_text) == 0)
        return;

    lv_obj_set_style_bg_color(view->dot,lv_color_hex(color),LV_PART_MAIN);
    lv_label_set_text(view->state_label,state_text);
    lv_obj_set_style_text_color(view->state_label,lv_color_hex(color),0);
    view->cached_color = color;
    strncpy(view->cached_text,state_text,sizeof(view->cached_text) - 1);
    view->cached_text[sizeof(view->cached_text) - 1] = '\0';
}
...
static void refresh_node_status_view(void)
{
    const gateway_state_t *state = gateway_state_get();
    int imx6ull_online = gateway_state_imx6ull_online(state);

    if(imx6ull_online){
        set_node_status(NODE_STATUS_IMX6ULL,"ONLINE",NODE_COLOR_ONLINE);
    }else{
        set_node_status(NODE_STATUS_IMX6ULL,"OFFLINE",NODE_COLOR_OFFLINE);
    }

    if(!imx6ull_online){
        set_node_status(NODE_STATUS_STM32,"UNKNOWN",NODE_COLOR_UNKNOWN);
    }else if(gateway_state_stm32_online(state)){
        set_node_status(NODE_STATUS_STM32,"ONLINE",NODE_COLOR_ONLINE);
    }else{
        set_node_status(NODE_STATUS_STM32,"OFFLINE",NODE_COLOR_OFFLINE);
    }
}

static void init_node_status_item(lv_obj_t *parent,
                                  node_status_id_t id,
                                  const char *node_name)
{
    node_status_view_t *view = &node_status_views[id];
    lv_obj_t *item = lv_obj_create(parent);
    lv_obj_t *name_label;

    view->cached_color = 0xFFFFFFFFu;
    view->cached_text[0] = '\0';

    lv_obj_set_size(item,150,44);
    lv_obj_add_style(item,&com_style,0);
    lv_obj_set_style_bg_opa(item,LV_OPA_0,LV_PART_MAIN);
    lv_obj_clear_flag(item,LV_OBJ_FLAG_SCROLLABLE);

    view->dot = lv_obj_create(item);
    lv_obj_set_size(view->dot,10,10);
    lv_obj_set_style_radius(view->dot,LV_RADIUS_CIRCLE,LV_PART_MAIN);
    lv_obj_set_style_border_width(view->dot,0,LV_PART_MAIN);
    lv_obj_set_style_bg_opa(view->dot,LV_OPA_COVER,LV_PART_MAIN);
    lv_obj_align(view->dot,LV_ALIGN_LEFT_MID,0,0);

    name_label = lv_label_create(item);
    obj_font_set(name_label,FONT_TYPE_LETTER,FONT_SIZE_TEXT_4);
    lv_label_set_text(name_label,node_name);
    lv_obj_set_style_text_color(name_label,APP_COLOR_WHITE,0);
    lv_obj_align(name_label,LV_ALIGN_TOP_LEFT,20,0);

    view->state_label = lv_label_create(item);
    obj_font_set(view->state_label,FONT_TYPE_LETTER,FONT_SIZE_TEXT_5);
    lv_obj_align(view->state_label,LV_ALIGN_BOTTOM_LEFT,20,0);
    set_node_status(id,"UNKNOWN",NODE_COLOR_UNKNOWN);
}

static lv_obj_t *init_node_status_view(lv_obj_t *parent)
{
    lv_obj_t *container = lv_obj_create(parent);

    lv_obj_set_size(container,330,50);
    lv_obj_add_style(container,&com_style,0);
    ...
    init_node_status_item(container,NODE_STATUS_IMX6ULL,"i.MX6ULL");
    init_node_status_item(container,NODE_STATUS_STM32,"STM32 CAN");
    lv_obj_align(container,LV_ALIGN_BOTTOM_RIGHT,0,-5);
    lv_obj_move_foreground(container);

    refresh_node_status_view();
    return container;
}
...
static void refresh(lv_timer_t *timer){
    ...
    if(page_type == TIME_TYPE_1 || page_type == TIME_TYPE_2){
        ...
        if(page_type == TIME_TYPE_1){
            if(set_label_text_if_changed(weather_label,device_state->weather_info))
                lv_obj_align_to(weather_label,time_label,LV_ALIGN_OUT_BOTTOM_MID,0,10);
        }
    }else if(page_type == TIME_TYPE_3){
        if(active_clock != NULL && active_clock->dial_img != NULL)
    ...
    if(refresh_cache.wifi_connected !=
       (device_state->wifi_connect_state == WPA_WIFI_CONNECT)){
        refresh_cache.wifi_connected =
            device_state->wifi_connect_state == WPA_WIFI_CONNECT;
        lv_img_set_src(wifi_img,refresh_cache.wifi_connected ?
                       GET_IMAGE_PATH("icon_wifi_connect.png") :
                       GET_IMAGE_PATH("icon_wifi_disconnect.png"));
    }
    if(refresh_cache.ble_connected != (device_state->ble_mesh_state == CONNECT)){
        refresh_cache.ble_connected = device_state->ble_mesh_state == CONNECT;
        lv_img_set_src(ble_img,refresh_cache.ble_connected ?
                       GET_IMAGE_PATH("icon_ble_connect.png") :
                       GET_IMAGE_PATH("icon_ble_disconnect.png"));
    }
    if(refresh_cache.volume_muted != (device_state->volume_value == 0)){
        refresh_cache.volume_muted = device_state->volume_value == 0;
        lv_img_set_src(vol_img,refresh_cache.volume_muted ?
                       GET_IMAGE_PATH("icon_volx.png") :
                       GET_IMAGE_PATH("icon_vol.png"));
    }
    if(refresh_cache.alarm_active !=
       (device_state->is_open_type1 || device_state->is_open_type2)){
        refresh_cache.alarm_active =
            device_state->is_open_type1 || device_state->is_open_type2;
        lv_img_set_src(alarm_img,refresh_cache.alarm_active ?
                       GET_IMAGE_PATH("icon_alarm_state.png") :
                       GET_IMAGE_PATH(""));
    }
    refresh_node_status_view();
}
...
static lv_obj_t * init_user_avatar(lv_obj_t *parent){
    ...
    lv_obj_set_style_pad_left(user_img,70,LV_PART_MAIN);
    return user_img;
}
...
static lv_obj_t * init_menu_list(lv_obj_t *parent){
    ...
    for(int index = 0;index < length;index ++){
        if(!menu_item_visible(menu_info_list[index].type)){
            continue;
        }
...
static lv_obj_t * init_maple_label(lv_obj_t *parent){
    (void)parent;
    lv_obj_t *label = lv_label_create(lv_scr_act());
    obj_font_set(label,FONT_TYPE_LETTER, FONT_SIZE_TEXT_1);
    lv_label_set_text(label,"Maple");
    lv_obj_set_style_text_color(label,APP_COLOR_WHITE,0);
...
{
    node_status_views[NODE_STATUS_IMX6ULL].dot = NULL;
    node_status_views[NODE_STATUS_IMX6ULL].state_label = NULL;
    node_status_views[NODE_STATUS_STM32].dot = NULL;
    node_status_views[NODE_STATUS_STM32].state_label = NULL;
    active_clock = NULL;
    refresh_cache.wifi_connected = -1;
    refresh_cache.ble_connected = -1;
    refresh_cache.volume_muted = -1;
    refresh_cache.alarm_active = -1;
    ...
    lv_obj_align_to(info_obj,user_avatar_obj,LV_ALIGN_OUT_RIGHT_MID,60,
                    page_type == TIME_TYPE_2 ? -20 : 0);
    ...
    init_maple_label(cont);
    init_node_status_view(cont);
